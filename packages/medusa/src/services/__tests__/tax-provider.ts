...
import { RegionModelMock } from "../../repositories/__mocks__/region"
...
describe("TaxProviderService", () => {
  describe("retrieveProvider", () => {
    ...
    it("successfully retrieves system provider", async () => {
      const region = await RegionModelMock.create({ tax_provider_id: null, })
      const provider = providerService.retrieveProvider(region)
      expect(provider).toEqual("system")
    })

    it("successfully retrieves external provider", async () => {
      const region = await RegionModelMock.create({ tax_provider_id: "test", })
      const provider = providerService.retrieveProvider(region)
    ...
    it("fails when payment provider not found", () => {
      ...
      try {
        providerService.retrieveProvider(RegionModelMock.create({ tax_provider_id: "unregistered" }))
      } catch (err) {
  ...
  describe("getTaxLines", () => {
    ...
    test("success", async () => {
      const providerService = new TaxProviderService(container)
      providerService.getRegionRatesForProduct = jest.fn(async () => [])
